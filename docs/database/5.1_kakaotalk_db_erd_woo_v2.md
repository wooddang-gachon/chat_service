...
erDiagram
    ...
    GIFT_COUPONS ||--o| GIFT_COUPON_ARCHIVES : "archives_to"
...
### 2.8 기프티콘 상태 전이 및 탈퇴 시 법적 격리 보존 (8.1 & 2.1)
- 기프티콘 결제/선물 관련 도메인은 복잡한 쿠폰 상태 전이를 지원하기 위한 설계가 필요하며, 탈퇴 회원 정보와의 격리 보존을 위해 `GIFT_COUPON_ARCHIVES` 테이블을 활용합니다.
- 회원이 탈퇴할 때, 법적으로 5년간 보존해야 하는 결제/선물 이력 데이터를 `GIFT_COUPON_ARCHIVES` 테이블로 Hard Move(이관)시킵니다. 이때 회원의 실제 개인정보와 연결되는 FK 제약 조건을 완전히 끊어, 탈퇴 처리 시 발생하는 물리 정합성 오류를 예방하고 개인정보를 안전하게 격리 보호합니다.
...
- **법적 보존 데이터 아카이빙 (`GIFT_COUPON_ARCHIVES`)**: 탈퇴 회원의 거래 이력을 5년 동안 안전하게 이관 보존하기 위해 `GIFT_COUPON_ARCHIVES`를 설계했습니다. 이는 RDB의 무결성(FK) 제약에 묶이지 않는 비식별 관계의 독립적 로그성 테이블로 동작하여 개인정보 분리 파기와 법적 보존 의무를 완벽히 호환시킵니다.
...
### 2.22 생일자 친구 목록 조회 성능 최적화 (16.1)
- **생일 데이터 분리 (`USER_BIRTHDAYS` 테이블 신설)**: 기존 `PROFILES` 테이블에 포함되어 있던 생년월일 필드 중 생일 조회에 불필요한 속성들을 분리하고 전용 테이블을 구축했습니다.
- **(birth_month, birth_day) 복합 인덱스 설계**: 오늘 생일인 친구 목록을 효율적으로 색인하기 위해 월(`birth_month`)과 일(`birth_day`) 컬럼에 복합 B-Tree 인덱스를 설정했습니다. 이로써 `WHERE MONTH(birthday) = ?`와 같은 함수 사용으로 인한 인덱스 무효화 현상을 완벽히 해결하여 매일 수백 명의 친구 중 생일자를 O(log N)의 범위 스캔(Range Scan)으로 고속 조회합니다.
